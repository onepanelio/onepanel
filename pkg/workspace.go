...
func (c *Client) CreateWorkspace(namespace string, workspace *Workspace) (*Workspace, error) {
	...
	if len(workspace.Labels) > 0 {
		_, err = c.InsertLabelsBuilder(TypeWorkspace, workspace.ID, workspace.Labels).
			RunWith(c.DB).
			Exec()
		if err != nil {
			return nil, err
...
func (c *Client) ListWorkspaces(namespace string, paginator *pagination.PaginationRequest) (workspaces []*Workspace, err error) {
	...
	labelsMapped, err := c.GetDbLabelsMapped(TypeWorkspace, WorkspacesToIds(workspaces)...)
	if err != nil {
		return nil, err
	}

	for _, workspace := range workspaces {
		workspace.Labels = labelsMapped[workspace.ID]
	}
